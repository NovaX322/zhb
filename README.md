...
- 走 GitHub CDN：**不用登录、不用会员、不限速**。
- 也可以直接把 Release 里的附件直链贴给玩家。

### 为什么放在 Release，而不是仓库里？

GitHub 的**仓库单文件上限是 100 MB**，而整合包普遍 1 GB 起，直接传会被拒绝。**Release 附件上限是 2 GB**，所以本仓库的整合包一律以 Release 附件形式分发，仓库代码区只放说明文件。
