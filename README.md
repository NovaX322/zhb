...
- **非商业自用**：本仓库仅面向**自己开的服务器**的玩家提供下载，不售卖、不设付费墙、不接广告、不以此盈利，也不做对外推广。
- **原包与作者**：每个整合包都标注**原包出处与原作者**，并给出原包地址。请优先走原作者渠道下载。
- **搬运的包**：文件与原发布版本**逐字节一致**，附 SHA-256 供比对。
- **魔改的包**：标注为**「修改版」**，写明基于哪个原包与版本，并保留原作者署名与许可信息（如原包为 GPL-3.0 等）。
- **下架即删**：原作者或权利方要求下架，**第一时间删除，无需任何前置流程**。
---
...
侵权 / 版权 / 许可问题请见 **[TAKEDOWN.md](./TAKEDOWN.md)**（下架与版权响应）。
